...
var _getMarkedDays = require("./utils/getMarkedDays");
...
const Calendar = props => {
  ...
  const CMainComponent = (0, _classes.getClasses)(['calendar'], classPrefix);
  const [active, setActive] = React.useState(() => (0, _utils.getYearAndMonth)(date));
  ...
  const markedDays = React.useMemo(() => (0, _getMarkedDays.getMarkedDays)(markers), [markers]);
  const calendarData = React.useMemo(() => (0, _utils.getCalendarData)({
    active,
    selected,
    markers: markedDays,
    firstDayIsMonday
  ...
  const activeMonth = React.useMemo(() => active.year === current.year ? active.month : null, [active, current]);
  const switchMonth = React.useCallback(direction => {
    if (direction === 'prev') {
